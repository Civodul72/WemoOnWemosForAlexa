...
// Fonction Switch : Modification avec le type device
...
void Switch::startWebServer(){
  ...
  Serial.print("WebServer ON sur port: ");
  Serial.println(localPort);
}
 
void Switch::handleEventservice(){
  Serial.println("## Reponse pour /eventservice.xml ... ##\n");
  ...
  String eventservice_xml = "<scpd xmlns=\"urn:Belkin:service-1-0\">"
        "<actionList>"
          ...
          "<action>"
            "<name>GetTempValue</name>"
            "<argumentList>"
              "<argument>"
                "<retval/>"
                "<name>TempValue</name>"
                "<relatedStateVariable>TempValue</relatedStateVariable>"
                "<direction>out</direction>"
              "</argument>"
            "</argumentList>"
          "</action>"
...
void Switch::handleUpnpControl(){
  Serial.println("## Reponse pour /upnp/control/basicevent1 ... ##");      
...
//  }
  ...
  if(request.indexOf("SetBinaryState") >= 0) {
    ...
    if(request.indexOf("<BinaryState>1</BinaryState>") >= 0) {
        Serial.println("Requete Turn ON");
        switchStatus = onCallback();
        sendRelayState();
    }

    if(request.indexOf("<BinaryState>0</BinaryState>") >= 0) {
        Serial.println("Requete Turn OFF");
...
void Switch::handleRoot(){
  server->send(200, "text/plain", "Demandez a Alexa de decouvrir les peripheriques - Forum Alexa France");
}

void Switch::handleSetupXml(){
  Serial.println("## Reponse pour /setup.xml ... ##\n");
  ...
  String setup_xml = "<?xml version=\"1.0\"?>"
            "<root>"
             "<device>"
                ...
                "<serialNumber>221517K0101780</serialNumber>"
...
void Switch::respondToSearch(IPAddress& senderIP, unsigned int senderPort) {
  Serial.println("");
  Serial.print("Envoi reponse vers ");
  ...
  String response = 
       ...
       "01-NLS: b9200ebb-736d-4b93-bf03-835149d13984\r\n"
